 ...
 *          const members, access specifiers, storage qualifiers.
 * Returns null for lines that are not parseable declarations.
 */
export function parseFieldLine(line: string): ParsedField | null {
  ...
  // Normalize pointer: "int *ptr" → "int* ptr"
  s = s.replace(/(\w)\s*\*\s*([A-Za-z_])/, '$1* $2');

  // Variable name = last valid C++ identifier
